...
## remove any individual species observations that are juveniles (< 25% of maximum adult length)

 x$cutoff<-x$LMAX*0.25
 a<-which(x$SIZE_ < x$cutoff)
 x<-x[-a,]
 
 ## remove any species that are < 50 cm TL that we see on tow
 ## RETURN HERE - PREPARE TOW DATA
...
wd<-droplevels(x)
save(wd, file="data/TMPwd.Rdata")  #Save clean working data
...
save(sectors, file="data/TMPsectors.Rdata")  # save cleaned up sectors file
